...
const SESSION = 'anorChineseCurrentStudent';
const TEST_ORDER = [
  { key: 'Grammatika', file: 'grammar.html', label: 'Grammar' },
  { key: 'Tinglash', file: 'listening.html', label: 'Listening' },
  { key: 'O‘qish', file: 'reading.html', label: 'Reading' },
  { key: 'Yozish', file: 'writing.html', label: 'Writing' },
  { key: 'Gapirish', file: 'speaking.html', label: 'Speaking' },
];
const LANGUAGE_STORE = 'anorChineseLanguage';
...
const RUSSIAN = {
  ...
  'Anor School o‘quvchilari uchun xitoy tili bo‘yicha qulay va zamonaviy daraja aniqlash platformasi.': 'Современная платформа для определения уровня китайского языка учеников Anor School.',
  'SIZNING PROFILINGIZ': 'ВАШ ПРОФИЛЬ', 'xush kelibsiz!': 'добро пожаловать!', 'Xitoy tili diagnostikasi': 'Диагностика китайского языка',
  ...
  'Javoblaringizni xitoy tilida yozing. O‘qituvchi mazmun va grammatikani baholaydi.': 'Пишите ответы на китайском языке. Учитель оценит содержание и грамматику.', 'Ovoz yozuvingiz orqali talaffuz va og‘zaki nutqingiz tekshiriladi.': 'Произношение и устная речь будут проверены с помощью аудиозаписи.',
  ...
  'Telegramga yuborishda xatolik yuz berdi.': 'Ошибка при отправке в Telegram.', 'Xitoy tili diagnostikasi natijasi': 'Результат диагностики китайского языка',
...
async function showDashboard() {
  ...
  document.querySelector('#studentClass').textContent = `${activeStudent.schoolClass} ${t('sinf')} · ${t('Xitoy tili diagnostikasi')}`;
...
async function renderDynamicTest() {
  ...
  if (section === 'Gapirish') {
    ...
    if (!questions.length) { form.innerHTML = noQuestionsMarkup(); return; }
    form.innerHTML = `<div class="question"><span class="question-number">${t('OG‘ZAKI TOPSHIRIQLAR')}</span>${questions.map((question, index) => `<div class="writing-prompt"><strong>${index + 1}-${t('topshiriq')}.</strong> ${escapeHtml(t(question.prompt))}</div>`).join('')}<p><strong>${t('Baholash:')}</strong> ${t('talaffuz va tonlar, gapning aniqligi, so‘z boyligi hamda nutq ravonligi.')}</p><div class="record-area"><button id="recordButton" class="record-button" type="button" aria-label="${t('Ovozni yozish')}">●</button><p id="recordStatus" class="record-status">${t('Yozishni boshlash uchun mikrofon tugmasini bosing.')}</p><audio id="audioPreview" class="audio-preview hidden" controls></audio></div></div><button class="primary-button submit-test speaking-theme" type="submit">${t('Og‘zaki javobni yuborish →')}</button><div id="testResult" class="result-box"></div>`;
  }
}
...
async function renderQuestionFields() {
  ...
  if (passageManager) {
    if (section === 'O‘qish') {
      ...
      passageManager.innerHTML = `<div class="reading-passage-manager"><p class="manager-note">${t('Bu matn o‘quvchiga barcha Reading savollaridan oldin ko‘rinadi.')}</p><label>${t('Asosiy matn')}<textarea id="readingPassageContent" rows="5" placeholder="我叫李华……">${escapeHtml(passage.content || '')}</textarea></label><label>${t('Tarjima (ixtiyoriy)')}<textarea id="readingPassageTranslation" rows="3" placeholder="Matnning o‘zbekcha tarjimasi">${escapeHtml(passage.translation || '')}</textarea></label><button id="saveReadingPassage" class="primary-button dark-button" type="button">${t('Matnni saqlash')}</button><p id="readingPassageMessage" class="form-message" role="status"></p></div>`;
...
function speakEnglish(button) { const phrase = button.dataset.text; if (!('speechSynthesis' in window)) return; window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(phrase); utterance.lang = 'en-US'; utterance.rate = .82; window.speechSynthesis.speak(utterance); }
...
function makePdf(student) {
  ...
  results.forEach((result, index) => {
    ...
    stream.push(fill + ' rg', `57 ${y} 481 47 re f`, '0.77 0.58 0.22 rg', `57 ${y} 5 47 re f`,
      '0.13 0.22 0.29 rg', 'BT', '/F2 12 Tf', `75 ${y + 19} Td`, `(${pdfSafe(TEST_ORDER[index].label)}) Tj`, '/F1 9 Tf', '0 -13 Td', `(English language skill ${index + 1}) Tj`, 'ET',
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.querySelectorAll('.play-audio').forEach(button => button.addEventListener('click', () => speakEnglish(button)));
